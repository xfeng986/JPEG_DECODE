...
#if defined(HAVE_GOWORLD_GWMTF9360A_MODIFY)
...
INT vpostLCMInit_GOWORLD_GWMTF9360A_modify(PLCDFORMATEX plcdformatex, UINT32 *pFramebuf)
{
	/* 1st line is diplayed wrong, so 1st is displayed in 2nd line */
//	volatile S_DRVVPOST_SYNCLCM_WINDOW sWindow = {960,240,320};	
	volatile S_DRVVPOST_SYNCLCM_WINDOW sWindow = {960,241,320};	
	volatile S_DRVVPOST_SYNCLCM_HTIMING sHTiming = {1,203,(UINT8)60};
//	volatile S_DRVVPOST_SYNCLCM_VTIMING sVTiming = {1,19,4};
	volatile S_DRVVPOST_SYNCLCM_VTIMING sVTiming = {1,18,4};
...
//	u32PLLclk = sysGetPLLOutputKhz(eSYS_UPLL, 12000);	// CLK_IN = 12 MHz
    ...
    outpw(REG_LCM_FSADDR, inpw(REG_LCM_FSADDR) - 640);    
#endif    
...
INT32 vpostLCMDeinit_GOWORLD_GWMTF9360A_modify(VOID)
{
	vpostVAStopTrigger();
	vpostFreeVABuffer();
