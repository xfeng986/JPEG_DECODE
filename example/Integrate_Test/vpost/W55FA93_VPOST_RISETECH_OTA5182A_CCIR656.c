 ...
 * 
 * FILENAME
 *     FA93_VPOST_RISETECH_OTA5182A_CCIR656.c
 *
 * VERSION
 *     0.1 
 *
...
void RISETECH_OTA5182A_CCIR656_Init(void)
{
#if 1	// WF20 board
	outpw(REG_GPDFUN, inpw(REG_GPDFUN) & ~0xCF000000);			// set GPB_15/GPB_13/GPB_12 to be GPIO pins	
	
	// set output mode
	outpw(REG_GPIOD_OMD, inpw(REG_GPIOD_OMD) | (BIT15+BIT12+BIT13));	// set GPB_14/GPB_13 to be output mode

	// set internal pull-up resistor
	outpw(REG_GPIOD_PUEN, inpw(REG_GPIOD_PUEN) | (BIT15+BIT13+BIT12));// set GPB_14/GPB_13 to be internal pullup
#else
...
INT vpostLCMInit_RISETECH_OTA5182A_CCIR656(PLCDFORMATEX plcdformatex, UINT32 *pFramebuf)
{
	volatile S_DRVVPOST_SYNCLCM_WINDOW sWindow = {320,240,320};	
//	volatile S_DRVVPOST_SYNCLCM_WINDOW sWindow = {640,480,240};	
	volatile S_DRVVPOST_SYNCLCM_HTIMING sHTiming = {1,93,(UINT8)5};
...
//	vpostSetLCM_TypeSelect(eDRVVPOST_HIGH_RESOLUTINO_SYNC);	
	...
	RISETECH_OTA5182A_CCIR656_Init();
	
	outpw(REG_LCM_TVCtl, inpw(REG_LCM_TVCtl) & (~TVCtl_FBSIZE));	
...
INT32 vpostLCMDeinit_RISETECH_OTA5182A_CCIR656(VOID)
{
	vpostVAStopTrigger();
	vpostFreeVABuffer();
...
#endif    //HAVE_RISETECH_OTA5182A_CCIR656
